...
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
print("Run MMP Mutation Scan Pipeline for ALL BAM files in a directory")
print("--------------------------------------------------------------------------------")
print("Target directory:", bam_dir)
print("BAM files discovered:", n_bam_paths)

std_cmd = "python run_mutation-scan.py -b %s -i %s -f %f"
if downsample:
    std_cmd += " -d"
    
for i, bam_path in enumerate(bam_paths):
    print("Running BAM file:", i)
    cmd = std_cmd % (bam_path, gene_ini, min_freq)
    print(cmd)
    os.system(cmd)
